...
angular.module('grafterizerApp')
  .controller('InputFormatterUploadedController', function ($scope, $mdDialog, transformationDataModel, $state, generateClojure, $rootScope) {
  ...
  $scope.selectedType = "CSV";
  $scope.delimiter = ',';

  // uploads files according to given inputs
  $scope.proceed = function () {
    // always the first step is read dataset
    var uploadFunction = new transformationDataModel.UploadDatasetFunction($scope.selectedType, $scope.typeList, $scope.delimiter, null, $scope.selectedSheet, $scope.extension, 'Reads the input data for the data transformation. \n Cannot be moved or removed');
    ...
    if ($scope.transformation.pipelines[0].functions.length !== 0 && $scope.transformation.pipelines[0].functions[0].__type !== "UploadDatasetFunction"){
      ...
      $scope.transformation.pipelines[0].functions[0] = uploadFunction; // always the first step is read dataset
    }
